...
class TestBasicCRUDOperations:
    ...
    def test_update_patient(self, fhir_server_with_requests_mock):
        ...
        create_response = requests.post(f"{server.base_url}/Patient", json=initial_patient)
        patient_id = create_response.json()['created_resource']['id']
        ...
        update_response = requests.put(f"{server.base_url}/Patient/{patient_id}", json=updated_patient)
...
class TestPatientIdentifiers:
    ...
    def test_search_patient_by_identifier(self, fhir_server_with_requests_mock):
        ...
        patients = [
            ...
            {
                "resourceType": "Patient",
                "identifier": [{"system": "http://hospital.org/mrn", "value": "MRN002"}],
        ...
        response = requests.get(f"{server.base_url}/Patient?identifier=http://hospital.org/mrn|MRN001")
    ...
    def test_search_patient_by_identifier_value_only(self, fhir_server_with_requests_mock):
...
class TestConditionalOperations:
    ...
    def test_conditional_create_existing_patient(self, fhir_server_with_requests_mock):
        ...
        assert result['created_resource']['name'][0]['family'] == 'Existing'  # Original name
    ...
    def test_conditional_update_nonexistent_patient(self, fhir_server_with_requests_mock):
        """Test conditional update when patient doesn't exist"""
        server = fhir_server_with_requests_mock

        new_patient = {
            "resourceType": "Patient",
            "identifier": [{"system": "http://hospital.org/mrn", "value": "NEWUPDATE123"}],
...
class TestMultipleResourceTypes:
    ...
    def test_create_observation(self, fhir_server_with_requests_mock):
        ...
        response = requests.post(f"{server.base_url}/Observation", json=observation_data)
    ...
    def test_create_practitioner(self, fhir_server_with_requests_mock):
        ...
        response = requests.post(f"{server.base_url}/Practitioner", json=practitioner_data)
...
class TestFhirResourcesIntegration:
    ...
    def test_create_with_fhir_resources_model(self, fhir_server_with_requests_mock):
        ...
        patient = Patient(
            identifier=[
                Identifier(system="http://example.org/mrn", value="FHIR123")
            ],
            name=[
                HumanName(family="Model", given=["FHIR"])
            ]
        )
    ...
    def test_direct_model_storage(self, mock_fhir_server):
        ...
        retrieved_model = server.read_resource('Patient', patient_id, return_fhir_model=True)
