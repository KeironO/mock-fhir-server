...
class TestConditionalOperations:
    ...
    def test_conditional_create_with_if_none_exist(
        ...
        assert (
            first_result["created_resource"]["id"]
            == second_result["created_resource"]["id"]
        )
    ...
    def test_regular_update(self, fhir_server_with_requests_mock):
        ...
        response = requests.put(
            f"{server.base_url}/Patient/{patient_id}", json=updated_data
        )
...
class TestBundleOperations:
    ...
    def test_bundle_with_updates(self, fhir_server_with_requests_mock):
        ...
        patient = {
            "resourceType": "Patient",
            "identifier": [{"system": "http://example.org/mrn", "value": "54321"}],
            ...
            "entry": [
                {
                    ...
                    "resource": {
                        "resourceType": "Patient",
                        "identifier": [
                            {"system": "http://example.org/mrn", "value": "54321"}
                        ],
                ...
                },
                {
                    ...
                    "resource": {
                        "resourceType": "Patient",
                        "identifier": [
                            {"system": "http://example.org/mrn", "value": "99999"}
                        ],
            ...
            "entry": [
                # Conditional create - should find existing
                {
                    ...
                    "resource": {
                        "resourceType": "Patient",
                        "identifier": [
                            {"system": "http://example.org/mrn", "value": "11111"}
                        ],
...
class TestAdvancedSearching:
    ...
    def test_search_with_multiple_identifiers(self, fhir_server_with_requests_mock):
        ...
        patients = [
            ...
            {
                "resourceType": "Patient",
                "identifier": [{"system": "http://example.org/mrn", "value": "MRN789"}],
        ...
        response = requests.get(
            f"{server.base_url}/Patient?identifier=http://example.org/mrn|MRN123"
        )
        assert response.status_code == 200
        ...
        response = requests.get(
            f"{server.base_url}/Patient?identifier=http://example.org/ssn|SSN456"
        )
        assert response.status_code == 200
        bundle = response.json()
        assert bundle["total"] == 1
